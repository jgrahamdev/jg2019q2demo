  ...
  protected function rewriteDirectoryConfig($extension, $rewrite_dir, $langcode = NULL) {
    ...
    if (file_exists($rewrite_dir) && $files = $this->fileScanDirectory($rewrite_dir, '/^.*\.yml$/i', ['recurse' => FALSE])) {
      foreach ($files as $file) {
        ...
        // Check for an empty original data accompanied by a langcode.
        // In this case, we assume we're writing a new language config override.
        $lang_config_override = (empty($original_data) && $langcode ? true : false);

        // Check for non-empty original data that isn't identical to rewrite data.
        $config_rewrite = (!empty($original_data) && ($rewrite_data != $original_data) ? true : false);

        // Log when original data is the same as the rewrite data.
        if ($rewrite_data == $original_data) {
          $log = $langcode ? '@config (@langcode)' : '@config';
          $this->logger->notice($log . ' has no changes to rewrite.', [
            '@config' => $file->name,
            '@langcode' => $langcode
          ]);
        }

        // Log when the original data is non existent and can't be rewritten.
        if (empty($original_data) && !$langcode) {
          $this->logger->notice('@config does not exist to rewrite.', ['@config' => $file->name]);
        }

        // Only make changes in the case of a proper config rewrite or a language config override.
        if ($config_rewrite || $lang_config_override) {
        ...
        if (isset($rewrite)) {
          ...
          // Do not retain previous rewrite info for next iteration of the loop.
          unset($rewrite);
          unset($config_rewrite);
          unset($lang_config_override);
        }
        else {
          $log = $langcode ? '@config (@langcode) not rewritten by @module' : '@config not rewritten by @module'; 
          $this->logger->notice($log, [
            '@config' => $file->name,
            '@module' => $extension->getName(),
            '@langcode' => $langcode
